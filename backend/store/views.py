...
import json
import base64
from .models import Category, Product, Order, OrderItem, PaymentProof, Shipment, Address, UPISettings, PasswordResetToken
...
def login_view(request):
    try:
        ...
        print(f"Login validation errors: {serializer.errors}")  # Debug print
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class AdminShipmentViewSet(viewsets.ModelViewSet):
    ...
    def create_delhivery_shipment(self, request, pk=None):
        ...
        try:
            ...
            # response = requests.post(delhivery_url, json=payload, headers=headers)
            # if response.status_code == 200:
            #     result = response.json()
            #     shipment.tracking_number = result.get('packages', [{}])[0].get('waybill')
            ...
            #     })
            # else:
            #     return Response({'error': 'Failed to create Delhivery shipment'}, status=400)
